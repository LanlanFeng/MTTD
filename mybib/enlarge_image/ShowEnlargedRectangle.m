...
% end
EnlargedShowStartRow = row - gap - LineWidth+2;
EnlargedShowStartColumn = 1 + gap + LineWidth+kuan-n-6;% 右下角显示放大图形
% EnlargedShowStartColumn = 1 + gap + LineWidth;% 左下角显示放大图形
for j = 1 : c
    I_rgb(EnlargedShowStartRow - m + 1:EnlargedShowStartRow,EnlargedShowStartColumn:EnlargedShowStartColumn + n - 1,j) = Enlarged(:,:,j); 
end
...
% end
% EnlargedShowStartRow = row - gap - LineWidth+2;
% % EnlargedShowStartColumn = 1 + gap + LineWidth+kuan-n-6;
