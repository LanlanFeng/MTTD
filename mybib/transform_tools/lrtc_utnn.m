function [X,obj,err,iter] = lrtc_utnn(UU,M,omega,opts)
...
if isfield(opts, 'Ttrue');      Ttrue= opts.Ttrue;        end
...
for iter = 1 : max_iter
    ...
    Ek = E;
    % update X
    [X,tnnX] = prox_utnn(UU,-E+M+Y/mu,1/mu); 
    % update E
    E = M-X+Y/mu;
    E(omega) = 0;
...
%     
    if iter==1 || mod(iter, 5) == 0
        if isfield(opts, 'Ttrue')
             RSE1= RSE(Ttrue,X);
            fprintf('TNN: iter = %d   RSE=%f   chg=%f  res=%f \n', iter, RSE1, chg,res);
        else
